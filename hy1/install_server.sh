...
rerun_with_sudo() {
  ...
  if has_prefix "$0" "/dev/fd/"; then
    ...
    if has_command curl; then
      curl -o "$_tmp_script" 'https://raw.githubusercontent.com/farukbrowser/hysteria-install/main/hy1/install_server.sh'
    elif has_command wget; then
      wget -O "$_tmp_script" 'https://raw.githubusercontent.com/farukbrowser/hysteria-install/main/hy1/install_server.sh'
    else
      return 127
    fi
